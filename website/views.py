...
def register_user(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, "You have now created an account")
            return redirect('home')
    else:
        form = SignUpForm()    
...
def customer_record(request, pk):
    if request.user.is_authenticated:
        ...
        messages.success(request,"You must be admin and be logged in")
        return redirect('home')
...
def delete_record(request, pk):
    if request.user.is_authenticated:
        ...
        delete_it.delete()
        messages.success(request, "Record Deleted succesful")
        return redirect('home')
    else:
        messages.success(request, "Record Deleted succesful")
        return redirect('home')
